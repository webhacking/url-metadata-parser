...
class MetaEntity {
    ...
    getContentByName(name) {
        return this.tags.map((tag) => tag.node.childNodes[0].attributes).filter(tag => tag.name && tag.name == name)[0] ? this.tags.map((tag) => tag.node.childNodes[0].attributes).filter(tag => tag.name && tag.name === name)[0].content : null;
    }
